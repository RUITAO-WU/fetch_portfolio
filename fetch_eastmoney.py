...
- site/{code}.html   (<table> with Date/Close)
...
def _extract_table_html(js_text: str) -> str:
    """Extract the first <table>...</table> from Eastmoney JS/HTML."""
...
def fetch_one(code: str) -> pd.DataFrame:
    ...
    # If header row is in the first data row, fix it.
...
def write_html_table(df: pd.DataFrame, code: str) -> Path:
    # PP 对 <table> 的首表、两列、英文字段名最稳定：Date/Close
